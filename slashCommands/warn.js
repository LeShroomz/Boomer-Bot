...
module.exports = {
  ...
  description: "Send warning to selected member from the server", //the command description for Slash Command Overview
  cooldown: 1,
  ...
  options: [ //OPTIONAL OPTIONS, make the array empty / dont add this option if you don't need options!	
		{"User": {name: "user", description: "Who are we warning?", required: true}},
  ...
  run: async (client, interaction) => {
    try{
		...
		if(target.roles.highest.position < member.roles.highest.position){
			...
			interaction.reply({content: `${target} was successfully warned for \`${reason}\``, ephemeral: true});
		} else {
